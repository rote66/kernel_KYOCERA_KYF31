...
static int __maybe_unused lock_policy_rwsem_##mode						\
(int cpu)								\
{									\
...
static void __maybe_unused unlock_policy_rwsem_read(int cpu)
{
	int policy_cpu = per_cpu(cpufreq_policy_cpu, cpu);
	BUG_ON(policy_cpu == -1);
...
static void bds_check_cpu(struct cpu_bds_info_s *this_bds_info)
{
	...
	unsigned int j;
	static unsigned int __maybe_unused phase = 0;
	static unsigned int __maybe_unused counter = 0;
	unsigned int __maybe_unused new_phase_max = 0;
