...
static void savagedzen_early_suspend(struct power_suspend *handler) {
        int i;
        suspended = 1;
...
static void savagedzen_late_resume(struct power_suspend *handler) {
        int i;
        suspended = 0;
        for_each_online_cpu(i)
...
static struct power_suspend savagedzen_power_suspend = {
...
static int __init cpufreq_savagedzen_init(void)
{
        ...
        register_power_suspend(&savagedzen_power_suspend);
